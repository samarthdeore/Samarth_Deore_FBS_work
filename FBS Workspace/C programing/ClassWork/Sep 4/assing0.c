#include<stdio.h>
double sub_persent(int ,int ,int ,int ,int );
int add(int,int);
...
{	
	...
	{	printf("\nEnter your Choice:");
		scanf("%d",&ch);
		if(ch==1){
			int math=58,sci=50,history=51,marathi=50,hindi=50;
			double x=sub_persent(math,sci,history,marathi,hindi);
			printf("total marks in persentage are %f",x);
		}
...
double sub_persent(int math,int sci,int history,int marathi,int hindi)
{
	
	double persentage,total_marks;
